...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for user in users:
            logger.debug(f'Inserting user: {user.username}')
            user.save()

        # Create teams
        team1 = Team(_id=ObjectId(), name='Blue Team')
        team2 = Team(_id=ObjectId(), name='Gold Team')
        logger.debug(f'Inserting team: {team1.name}')
        team1.save()
        logger.debug(f'Inserting team: {team2.name}')
        team2.save()
        ...
        for activity in activities:
            logger.debug(f'Inserting activity: {activity.activity_type}')
            activity.save()
        ...
        for entry in leaderboard_entries:
            logger.debug(f'Inserting leaderboard entry for user: {entry.user.username}')
            entry.save()
        ...
        for workout in workouts:
            logger.debug(f'Inserting workout: {workout.name}')
            workout.save()
